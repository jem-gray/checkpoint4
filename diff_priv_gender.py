...
df_gender = df_2.groupby(['current_complaint_category', 'gender'], as_index=False).count()
# print(df_gender)
# print(df_2.columns)
...
epsilon = [0.00075, 0.005, 0.0075, 0.01, 0.05, 0.075, 0.1, 0.5, 0.75, 1.0]
delta = 0
epsilon2 = list(map(str, epsilon))

# graph error rates for FEMALE, Failure to provide service
original_val = df_gender.iloc[2][2]
# print(original_val)
...
for i in range(10):
	...
	for val in epsilon:
		...
		result = reader.execute('SELECT current_complaint_category, gender, COUNT(*) AS num_complaints FROM public.df_2 GROUP BY gender, current_complaint_category')
		end_time.append(time.time())
		df_new = pd.DataFrame(result, columns =['current_complaint_category', 'gender', 'num_complaints'])
		df_new = df_new.iloc[1:]
		new_val = df_new.iloc[1][2]
